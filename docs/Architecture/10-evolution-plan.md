...
| 3.2 | **MoE (Mixtral).** ✅ **SHIPPED** | Routed mixture-of-experts selected by GGUF `expert_count`/`expert_used_count` (Mixtral's arch is `llama`, not a new `Arch`). Router → softmax over all experts → top-k → renormalize → weighted SwiGLU sum (`moe_ffn_row`), composed from the existing per-row `matmul` (no new `Backend` op; correct on every backend). 3-D expert stacks split per-expert at load; resident CUDA/GPU decode gated off for MoE. Validated by numeric routing-parity + synthetic-GGUF end-to-end (no real Mixtral GGUF in-repo). Detail: [`plans/phase-3-archs.md`](plans/phase-3-archs.md). Remaining: Qwen2-MoE shared experts, fused `MUL_MAT_ID`. | M |
