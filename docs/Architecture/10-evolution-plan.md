...
| 3.2 | **MoE (Mixtral + Qwen2-MoE).** ✅ **SHIPPED** | Routed mixture-of-experts selected by GGUF `expert_count`/`expert_used_count`. Router → softmax over all experts → top-k → weighted SwiGLU sum (`moe_ffn_row`), composed from the existing per-row `matmul` (no new `Backend` op). Mixtral (`Arch::Llama`) renormalizes the top-k weights; **Qwen2-MoE** (`Arch::Qwen2Moe`: QKV bias + NeoX rope) does NOT, and adds an always-on **shared expert** `sigmoid(gate·x)·SwiGLU_shexp(x)` at its own `n_ff_shexp`. 3-D expert stacks split per-expert at load; resident CUDA/GPU decode gated off for MoE. Validated by numeric routing/shared parity + synthetic-GGUF end-to-end (no real MoE GGUF in-repo). Detail: [`plans/phase-3-archs.md`](plans/phase-3-archs.md). Remaining: fused `MUL_MAT_ID`. | M+M |
