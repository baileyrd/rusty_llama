...
The arch is dispatched purely by the **name prefix** on metadata keys and the `blk.N.*`/`token_embd`/`output*` tensor names — there is no arch registry; `from_gguf` works for `llama` and anything sharing that naming. The tokenizer is loaded separately by `Tokenizer::from_gguf(gguf)` (called from `main.rs:66`), dispatching on `tokenizer.ggml.model` (`src/tokenizer.rs:55-60`): `"llama"` → SentencePiece (`Spm`), `"gpt2"` → byte-level BPE (`Bpe`), default `"llama"` when absent. Special tokens are read from the `tokenizer.ggml.token_type` array (`CONTROL = 3`, `USER_DEFINED = 4`; `src/tokenizer.rs:97-99`), with vocab/scores/merges/bos/eos from the other `tokenizer.ggml.*` keys. Details of what's done with this metadata are doc 07; what the hyper-params drive is doc 01.
...
- **Llama-arch only, name-prefix dispatch (not a registry).** `from_gguf` keys off `general.architecture` purely to build `{arch}.*` metadata keys and assumes Llama `blk.N.*` tensor naming (`src/model.rs:137-138, 197-233`); non-Llama architectures, MoE/expert tensors, and tensor-name remapping are unsupported. Only **6** `ggml_type`s parse (`src/quant.rs:43-57`); F16, BF16-as-anything-else, and the many other k-quants are rejected.
