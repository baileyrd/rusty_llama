...
| `attention` | `fn attention(&self, out: &mut [f32], q: &[f32], key_cache: &[f32], value_cache: &[f32], att: &mut [f32], pos: usize, n_heads: usize, n_kv_heads: usize, head_size: usize, seq_len: usize, kv_dim: usize, logit_softcap: f32)` | Grouped-query attention for the current position; scores `q` vs cached keys `0..=pos`, online-softmax (flash), value-weighted sum into `out`. `logit_softcap` is Gemma2 tanh score-capping (0.0 = off). `att` is caller scratch of length `n_heads*seq_len`, unused by the flash kernel and unspecified after the call (`mod.rs:96-111`) |
...
| `attention_batch(&self, out, q, key_cache, value_cache, att, pos_base, rows, n_heads, n_kv_heads, head_size, seq_len, kv_dim, logit_softcap)` | loops `self.attention`; reuses single-row `att` scratch across rows | `mod.rs:209-242` |

### `forward_step` / `forward_prefill`: the step contract

```rust
// REQUIRED — no default body (mod.rs:254)
fn forward_step(&self, model: &Model, state: &mut RunState, token: usize, pos: usize);

// DEFAULTED — mirrors the per-op model path (mod.rs:265-273)
...
`pub struct CpuBackend;` — `#[derive(Debug, Default, Clone, Copy)]`, a stateless zero-sized type; `CpuBackend::new()` returns `CpuBackend` and carries no state (`src/backend/cpu.rs:83-92`). Because it is a ZST, sharing `&self` across rayon threads is free and there is no per-thread allocation of backend state.

Two kernels are parallelized; the rest are tight scalar/elementwise loops the compiler autovectorizes:

- **`matmul`** — `out.par_iter_mut().enumerate().for_each(...)` splits the **output rows** across the rayon pool; each task computes one dot product (`src/backend/cpu.rs:106-135`; the int8 paths run through `dots_q8`/`dots_q8k`, `cpu.rs:41-63`). One row = one independent task, so there is no cross-task aliasing.
- **`attention`** — `out.par_chunks_mut(head_size).enumerate()` makes **one task per head**; `out` is sliced into disjoint per-head chunks so closures never alias (`src/backend/cpu.rs:388-390`). It is a **flash / online-softmax** kernel: each head streams the keys once with a running max `m` and exp-sum `l`, accumulating the value-weighted sum directly into `out_h` (no `seq_len` score buffer; `att` is unused). Grouped-query mapping: `kv_mul = n_heads / n_kv_heads`, `kv_off = (h / kv_mul) * head_size` (`src/backend/cpu.rs:383,393`).
- **`rmsnorm`** (`cpu.rs:95-104`), **`rope`** (`cpu.rs:305-360`), **`swiglu`** (`cpu.rs:427-432`), **`add`** (`cpu.rs:434-439`) are single-threaded loops — small/cheap relative to matmul. The inner loops use iterator `zip` chains over contiguous slices so LLVM autovectorizes them (build with `-C target-cpu=native`, see the module doc-comment `src/backend/cpu.rs:1-7`). `rope` rotates consecutive `(even, odd)` pairs per head, looking up the pair's precomputed `inv_freq[pair]`; pairs `< kv_dim` rotate both q and k, the rest rotate q only, and pairs past `inv_freq.len()` (partial rotary) pass through unchanged (`cpu.rs:337-359`); the per-pair cos/sin are precomputed once per call (`cpu.rs:328-336`).

`forward_step` is implemented as a thin delegate — the CPU per-op path is already efficient, so it just calls `crate::model::forward(model, state, self, token, pos)` (`src/backend/cpu.rs:441-450`). `CpuBackend` **does** override `matmul_batch` with a cache-blocked / register-tiled prefill GEMM (`src/backend/cpu.rs:189-302`) and `matmul_shared` to reuse one activation quant across q/k/v and gate/up (`cpu.rs:144-176`); it does not override `forward_prefill` or the other `*_batch` methods.
...
`#[cfg(test)] mod tests` (`src/backend/cpu.rs:453-1049`) holds correctness tests for every primitive plus three ignored timing/throughput benches (including a flash-vs-classic attention parity test and a `matmul_shared` bit-identity test). Detail belongs to `08-testing-benchmarking-parity.md`; at a glance:

| Test | What it pins | Cite |
|---|---|---|
| `matmul_f32_basic` | `W·x` numeric correctness on f32 path | `cpu.rs:461-470` |
| `matmul_quant_matches_f32` | Q8_0 matmul within tolerance of exact f32 | `cpu.rs:472-492` |
| `matmul_routes_kquants_to_int8` | Q4_K / Q6_K rows route to the int8 `vec_dot_*` path (output equals the kernel applied directly) | `cpu.rs:534-577` |
| `rmsnorm_*`, `swiglu_known_value`, `rope_*`, `attention_*` | per-primitive numeric behavior incl. RoPE partial-rotary / mscale / theta and flash-vs-classic attention parity | `cpu.rs:741-1048` |
| `bench_q8_0_matmul` *(ignored)* | dequant-row vs int8 `vec_dot_q8_0`, 2048×2048, prints **speedup ratio** | `cpu.rs:494-532` |
| `bench_q4_k_matmul` *(ignored)* | dequant-row vs int8 `vec_dot_q4_k` speedup | `cpu.rs:647-687` |
...
- **Cache-blocked CPU prefill GEMM.** `CpuBackend` overrides `matmul_batch` (`src/backend/cpu.rs:189-302`) with a cache-blocked, register-tiled GEMM that streams each weight row once and reuses it across all `rows` activations (toggle off with `RUSTY_LLAMA_NO_BLOCKED_GEMM`); it does not override `forward_prefill` or the other `*_batch` methods. The blocked GEMM is bit-identical to the per-row default. Fusion is reserved for the GPU/CUDA backends, whose overrides must match the CPU per-op result (`src/backend/mod.rs:202-214`). See `03-gpu-backend-wgpu.md` / `04-cuda-backend.md`.
...
- **`forward_step` is the only non-defaulted trait method** (`src/backend/mod.rs:254`) — a new backend can rely on defaults for everything else but must supply a step entry point.
- **`att` scratch contract is loose.** Callers must not depend on post-call contents of `att`; the CPU flash kernel leaves `att` untouched, other backends may not (`src/backend/mod.rs:83-86`). The `attention_batch` default reuses a single-row `att` across rows (`src/backend/mod.rs:215,226-233`).
